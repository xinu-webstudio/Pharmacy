...
import { MedicalProductInventoryModel, SalesAndPurchaseModel, MedicalProductsModel } from "../models/medicalProductInventory.model";
import { formatResponse } from "../../api/utilities/formatRes";
import { Response } from "express";
import { IMedicalProductInventory, ISaleAndPurchase, IMEDICALPRODUCTS } from "../types/medicalProductInventory.types";
...
const medicalProductController = {
  ...
  getAll: async (req: any, res: Response) => {
    try {
      const { limit = "20", page = "1", search, productCategory, ...rest } = req.query;
      ...
      return formatResponse(res, 200, true, "Medical Products retrieved successfully", {
        data: result,
        pagination: {
          currentPage,
          totalPages,
  ...
  getById: async (req: any, res: Response) => {
    try {
      ...
      return formatResponse(res, 200, true, "Medical Product retrieved successfully", result);
    } catch (error) {
      Logger.error("Medical Product retrieval error:", error);
      return formatResponse(
        res,
...
const updatePharmacyInventory = async (inventoryId: string, quantityChange: number, userId: any) => {
  try {
...
const createPharmacyInventory = async (data: IMedicalProductInventory, userId: any) => {
...
const medicalInventoryController = {
  ...
  getAll: async (req: any, res: Response) => {
    try {
      const { limit = "20", page = "1", search, status, category, ...rest } = req.query;
      const limittt = Math.max(parseInt(limit, 10), 1);
      const currentPage = Math.max(parseInt(page, 10), 1);
      const skip = (currentPage - 1) * limittt;
      ...
      return formatResponse(res, 200, true, "Medical Inventory retrieved successfully", {
        data: result,
        pagination: {
          currentPage,
          totalPages,
